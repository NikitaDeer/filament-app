    ...
    <div class="space-y-8 md:grid md:grid-cols-2 md:gap-12 md:space-y-0 lg:grid-cols-3">
      @forelse ($tariffs as $tariff)
        <div>
          ...
          <h3 class="mb-2 text-xl font-bold text-violet-200">{{ $tariff->title }}</h3>
          <h4 class="mb-2 text-xs font-bold text-violet-200">{{ $tariff->price }} &#8381;</h4>
          <p class="text-violet-200">{!! $tariff->description !!}</p>
        </div>
      @empty
        <x-site.no-items />
      @endforelse
