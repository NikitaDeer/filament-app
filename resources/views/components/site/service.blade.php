  ...
  <div class="mx-auto max-w-screen-xl px-6 lg:px-16">
    <div class="space-y-8 md:grid md:grid-cols-2 md:gap-12 md:space-y-0 lg:grid-cols-3">
      @forelse ($tariffs as $tariff)
        <div class="bg-gradient-to-tl from-violet-500 to-violet-300 p-6 rounded-xl shadow-xl hover:scale-105 transition-all duration-300">
          ...
          <h3 class="mb-4 text-3xl font-extrabold text-gray-800">{{
            $tariff->title }}</h3>
          <h4 class="mb-6 text-2xl font-bold text-yellow-400">{{ $tariff->price }} &#8381;</h4>
          <p class="text-gray-700 leading-relaxed text-lg">{!! $tariff->description !!}</p>
          <a href="#" class="mt-6 inline-block rounded-full bg-violet-600 px-6 py-3 text-lg font-semibold text-white hover:bg-violet-700 transition duration-300">
            Подробнее
          </a>
        </div>
      @empty
